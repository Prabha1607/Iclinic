    ...
    <div className="bg-white rounded-2xl border border-blue-50 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="space-y-1.5">
          ...
          {appt.notes && (
            <div className="flex items-start gap-2">
              <span className="text-xs text-slate-400 w-20 flex-shrink-0 pt-0.5">Notes</span>
              <span className="text-xs text-slate-500 italic">{appt.notes}</span>
            </div>
          )}
          {appt.instructions && (
            <div className="flex items-start gap-2">
              <span className="text-xs text-slate-400 w-20 flex-shrink-0 pt-0.5">Instructions</span>
              <span className="text-xs text-slate-600">{appt.instructions}</span>
            </div>
          )}
        </div>

        {/* Footer — booking channel + ID */}
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
          <span className="text-[10px] text-slate-300">#{appt.id}</span>
          {ch && (
            <span
