...
function Step1Patient({ onSelect, selected }: { onSelect: (p: Patient) => void; selected: Patient | null }) {
  ...
  const handleCreate = async () => {
    const allTouched = { first_name: true, last_name: true, email: true, phone_no: true, password: true };
    ...
    setCreating(true);
    try {
      const p = await createPatient({ ...form, role_id: 1 });
      setCreated(p);
      onSelect(p);
    }
  ...
  const handleReset = () => {
    setForm({ first_name: "", last_name: "", country_code: "+91", phone_no: "", email: "", password: "" });
    setCreated(null);
    setFieldErrors({});
    setTouched({});
    setServerError("");
    ...
    <div className="h-full flex flex-col gap-6">
      ...
      {mode === "search" ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 flex-1">
          <div>
            <FieldRow label="First Name" required>
              <input className={errCls(inputCls, fieldErrors.first_name)} placeholder="John" value={form.first_name}
                ...
                onBlur={() => handleFormBlur("first_name")} />
            </FieldRow>
          ...
          <div>
            <FieldRow label="Last Name" required>
              <input className={errCls(inputCls, fieldErrors.last_name)} placeholder="Doe" value={form.last_name}
                ...
                onBlur={() => handleFormBlur("last_name")} />
            </FieldRow>
          ...
          <div>
            <FieldRow label="Phone Number" required>
              <input className={errCls(inputCls, fieldErrors.phone_no)} placeholder="9876543210" value={form.phone_no}
                ...
                onBlur={() => handleFormBlur("phone_no")} />
            </FieldRow>
          ...
          <div className="sm:col-span-2">
            <FieldRow label="Password" required>
              <input className={errCls(inputCls, fieldErrors.password)} type="password"
                ...
                value={form.password} disabled={!!created}
          ...
          {serverError && (
            <div className="sm:col-span-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-4 py-3">{serverError}</div>
          )}
          ...
          {created ? (
            <div className="sm:col-span-2 space-y-3">
              ...
              <button disabled
                className="w-full py-3.5 bg-slate-100 text-slate-400 text-sm font-bold rounded-xl cursor-not-allowed border border-slate-200">
                ✓ Patient Created
              </button>
              {/* Reset to create another */}
              <button onClick={handleReset}
                className="w-full py-3 text-sm font-semibold text-[#3b5bfc] border border-[#3b5bfc]/30 rounded-xl hover:bg-[#eef2ff] transition">
